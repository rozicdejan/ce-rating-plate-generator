"""
Laser Label Generator
=====================
Full Streamlit app with persistent tool inventory storage.

Storage layout (created automatically next to this script):
  laser_labels_data/
    inventory.json   — merged tool inventory (all sources)
    uploads/         — copies of every uploaded file
"""
...
# ============================================================
# InventoryStore — persistent JSON-backed tool store
# ============================================================

class InventoryStore:
    """
    Persistent inventory stored as JSON.
      ...
      "tools": [
        {
          ...
          "source_hash":     str,   # SHA-256 of the file bytes
          "added_at":        str,   # ISO datetime
          "updated_at":      str,
          "priority":        int    # 1 = highest, lower = older
        },
        ...
      ],
      "files": [
    ...
    def file_known(self, file_hash: str) -> bool:
        """Return True if a file with this exact hash has already been imported."""
    ...
    def as_dataframe(self) -> pd.DataFrame:
        if not self._data["tools"]:
            return pd.DataFrame(columns=[
                "priority", "tool_number", "property_of",
                "part_description", "source_file", "added_at", "updated_at",
            ])
        df = pd.DataFrame(self._data["tools"])
        # Sort: lowest priority number = most important (top)
    ...
    def _next_priority(self) -> int:
        """Return priority value one step HIGHER than the current best (lower number = higher priority)."""
        if not self._data["tools"]:
            return 1
    ...
    def merge_from_dataframe(
        ...
        """
        Merge rows from df into the store.

        Rules
        -----
        1. If this exact file hash is already stored → skip entirely (duplicate upload).
        2. If a file with the SAME NAME but DIFFERENT hash exists → replace all
           rows that came from the old version of that file, then add new rows.
        3. New / updated rows get the highest priority (numerically lowest value).
        4. tool_number is the deduplication key within a single import:
           if two rows share a tool_number in the incoming file, keep the first.
        5. Across files: if the same tool_number exists from a different source file,
           update it (new file wins) and bump its priority.

        Returns a dict with counts: added, updated, unchanged, skipped.
        """
        now = datetime.now().isoformat(timespec="seconds")

        # Guard: exact same file already imported
        if self.file_known(source_hash):
            return {"added": 0, "updated": 0, "unchanged": 0, "skipped": "duplicate_hash"}

        # Remove all rows that came from an older version of the same filename
        removed_old = 0
        if self.filename_known(source_file):
            ...
            # Remove the old file record too
            self._data["files"] = [
                f for f in self._data["files"] if f.get("filename") != source_file
            ]

        # Build a lookup of existing tools by tool_number
        existing_by_num = {t["tool_number"]: t for t in self._data["tools"]}

        # Deduplicate incoming rows by tool_number (keep first occurrence)
        ...
        # Priority for all incoming rows sits above everything currently stored
        ...
        for i, row in enumerate(rows):
            ...
            priority = batch_priority_start - i   # each row in batch gets unique priority
            ...
            if tn in existing_by_num:
                ...
                if (
                    ...
                    unchanged += 1
                    # Still bump priority so new-file rows sort to top
    ...
    def delete_file_and_its_tools(self, filename: str):
        """Remove all tools imported from a given file AND the file record itself."""
...
# ============================================================
# Module-level store instance (cached across reruns)
# ============================================================
...
def save_upload_to_disk(data: bytes, filename: str) -> Path:
    ...
    # If same filename exists but different content, version it
    if dest.exists():
...
def get_mode_colors(mode: str):
    if mode == "Anodized aluminium (negative)":
        return {"plate_fill": "black", "text_fill": "white", "plate_stroke": "black",
                "hole_fill": "white", "guide_stroke": "#53b7ff", "preview_bg": "#dcdcdc"}
    return {"plate_fill": "white", "text_fill": "black", "plate_stroke": "black",
            "hole_fill": "white", "guide_stroke": "#1f77b4", "preview_bg": "#f3f3f3"}
...
def fit_text_block(
    text, desired_h_mm, box_x, box_y, box_w, box_h, weight,
    align="left", pad_x=0.0, max_lines=1,
    first_line_anchor_h=None, force_single_if_fits=False,
):
    raw_original = "" if text is None or pd.isna(text) else str(text).strip()
    raw_single   = normalize_space(raw_original.replace("\n", " "))

    empty = {"texts": [], "paths": [], "used_heights": [],
             "used_height_summary": 0.0, "line_boxes": [], "line_count": 0}
    if not raw_original:
        return empty

    sf, sb, sbbox, ss, sh = fit_text_for_box(raw_single, desired_h_mm, box_w, box_h, weight)
    single_complete = sf == raw_single

    best = {
        "texts": [sf], "bases": [sb], "scales": [ss], "used_heights": [sh],
        "line_box_h": box_h, "gap": 0.0,
    ...
    if (
        ...
        if lbh >= MIN_TEXT_HEIGHT_MM:
            for l1, l2 in build_two_line_candidates(raw_original):
                t1, b1, _, s1, h1 = fit_text_for_box(l1, desired_h_mm, box_w, lbh, weight)
                t2, b2, _, s2, h2 = fit_text_for_box(l2, desired_h_mm, box_w, lbh, weight)
                if not t1 or not t2:
                    continue
                complete = t1 == l1 and t2 == l2
                score    = (
                    (2000 if complete else 0) + min(h1, h2) * 100
                    + len(t1) + len(t2) - abs(len(l1) - len(l2)) * 0.15
                )
                if complete and not single_complete:
                    score += 500
                if score > best["score"]:
                    best = {"texts": [t1, t2], "bases": [b1, b2], "scales": [s1, s2],
                            "used_heights": [h1, h2], "line_box_h": lbh, "gap": gap, "score": score}
    ...
    if len(best["texts"]) == 1:
        ...
        anchor_h  = min(first_line_anchor_h if first_line_anchor_h is not None else best["line_box_h"], box_h)
        flt       = box_y + max(0.0, (anchor_h - h1) / 2.0)
        slt       = flt + h1 + gap
        placed1   = (place_path_top_aligned(best["bases"][0], best["scales"][0], box_x, flt, box_w, align=align, pad_x=pad_x)
                     if best["bases"][0] is not None else None)
        placed2   = (place_path_top_aligned(best["bases"][1], best["scales"][1], box_x, slt, box_w, align=align, pad_x=pad_x)
                     if best["bases"][1] is not None else None)
        if placed1: paths.append(placed1)
        if placed2: paths.append(placed2)
        line_boxes.extend([(box_x, flt, box_w, h1), (box_x, slt, box_w, h2)])
        used_h = (slt - box_y) + h2

    return {"texts": best["texts"], "paths": paths, "used_heights": best["used_heights"],
            "used_height_summary": used_h, "line_boxes": line_boxes, "line_count": len(best["texts"])}
...
def mpl_path_to_svg_d(path_obj):
    ...
    for verts, code in path_obj.iter_segments():
        if   code == MplPath.MOVETO:    out.append(f"M {verts[0]:.4f} {verts[1]:.4f}")
        elif code == MplPath.LINETO:    out.append(f"L {verts[0]:.4f} {verts[1]:.4f}")
        elif code == MplPath.CURVE3:    out.append(f"Q {verts[0]:.4f} {verts[1]:.4f} {verts[2]:.4f} {verts[3]:.4f}")
        elif code == MplPath.CURVE4:    out.append(f"C {verts[0]:.4f} {verts[1]:.4f} {verts[2]:.4f} {verts[3]:.4f} {verts[4]:.4f} {verts[5]:.4f}")
        elif code == MplPath.CLOSEPOLY: out.append("Z")
    return " ".join(out)
...
def compute_label_layout(owner, tool_number, part_desc,
    ...
    lb1 = fit_text_block(rows[1][0], fs2, left_x,  row2_y, left_w,  ROW2_H,           "bold",    max_lines=1)
    ...
    if use_stacked:
        ...
        rb2a = fit_text_block(part_lines[0], fs3, stack["value_x"], stack["value_y"],                                     stack["value_w"], stack["per_line_h"], "regular", max_lines=1)
        rb2b = fit_text_block(part_lines[1], fs3, stack["value_x"], stack["value_y"] + stack["per_line_h"] + stack["line_gap"], stack["value_w"], stack["per_line_h"], "regular", max_lines=1) \
               if part_lines[1] else {"paths": [], "texts": [], "used_height_summary": 0.0, "line_boxes": [], "used_heights": []}
        row2 = {"stacked": True, "stack": stack, "part_lines": part_lines,
                "label_block": lb2, "line1_block": rb2a, "line2_block": rb2b}
    else:
        ph   = max(ROW3_H, LABEL_H - row3_y - max(border_offset, PART_STACK_BOTTOM_MARGIN))
        lb2  = fit_text_block("Part description:", fs3, left_x,  row3_y, left_w,  ROW3_H, "bold",    max_lines=1)
        rb2  = fit_text_block(part_desc,           fs3, right_x, row3_y, right_w, ph,     "regular", max_lines=2, first_line_anchor_h=ROW3_H)
        row2 = {"stacked": False, "left_block": lb2, "right_block": rb2}

    return {"use_stacked_part": use_stacked, "right_w": right_w, "rows": rows,
            "left_block_0": lb0, "right_block_0": rb0,
            "left_block_1": lb1, "right_block_1": rb1,
            "row2_blocks": row2,
...
def generate_svg(owner, tool_number, part_desc, mode,
    ...
    all_paths = []
    meta      = {"left_sizes": [], "right_sizes": [], "left_texts": [], "right_texts": [],
                 "right_w": right_w, "stacked_part": layout["use_stacked_part"]}

    def collect(block, side):
        for p in block["paths"]:
            d = mpl_path_to_svg_d(p)
            if d: all_paths.append(d)
        if side == "left":
    ...
    collect(layout["left_block_0"],  "left");  collect(layout["right_block_0"], "right")
    collect(layout["left_block_1"],  "left");  collect(layout["right_block_1"], "right")
    ...
    if row2["stacked"]:
        ...
        meta["right_sizes"].append(max(row2["line1_block"].get("used_height_summary", 0),
                                       row2["line2_block"].get("used_height_summary", 0)))
...
def generate_dxf(owner, tool_number, part_desc, mode,
    ...
    if show_border:
        add_rounded_rect_dxf(msp, border_offset, border_offset,
                             LABEL_W-2*border_offset, LABEL_H-2*border_offset, corner_r, "BORDER")
    ...
    if row2["stacked"]:
        emit(row2["label_block"]); emit(row2["line1_block"])
        if row2["line2_block"].get("texts"): emit(row2["line2_block"])
    else:
        emit(row2["left_block"]); emit(row2["right_block"])
...
def build_batch_zip(df, default_mode, default_hole_dia, default_hole_offset,
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        for idx, (_, row) in enumerate(df.iterrows()):
            ...
            kw     = dict(owner=owner, tool_number=tn, part_desc=pd_val, mode=mode,
            ...
            records.append({"file_base": base, "property_of": owner, "tool_number": tn,
                            "part_description": pd_val, "mode": mode,
                            "hole_dia": hd, "hole_offset": ho, "holes_exported": show_holes})
            if progress_bar:
...
def find_custom_excel_sheet_and_header(uploaded_file):
    ...
    if best_sheet is None or best_score < 6:
        raise ValueError("Could not detect custom Excel header. "
                         "Ensure columns: lastnik / orodje / sifra_orodja / izdelek.")
...
def combine_multirow_headers(raw_df, header_row, depth=3):
    ...
    for ci in range(raw_df.shape[1]):
        ...
        for r in range(header_row, min(header_row + depth, len(raw_df))):
            ...
            if pd.notna(v):
                t = str(v).strip()
                if t and t.lower() != "nan": parts.append(t)
...
def normalize_custom_template_df(data_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "napis_na" in df.columns:
        ...
        for col in parsed.columns: df[col] = parsed[col]
    else:
        df["parsed_property_of"] = df["parsed_tool_number"] = df["parsed_part_description"] = ""
    ...
    df["mode"] = DEFAULT_MODE; df["hole_dia"] = DEFAULT_HOLE_DIA; df["hole_offset"] = DEFAULT_HOLE_OFFSET
    return df
...
with st.sidebar:
    ...
    mode_default  = st.selectbox("Default engraving mode", ALLOWED_MODES,
                                  index=1 if DEFAULT_MODE == "Anodized aluminium (negative)" else 0)
    st.divider()
    st.markdown("**Export options**")
    show_border = st.checkbox("Include border", value=True)
    show_holes  = st.checkbox("Include mounting holes", value=True,
...
with tab_single:
    ...
    with rc:
        ...
        svg_out, meta = generate_svg(owner=owner, tool_number=tool_number, part_desc=part_desc,
                                     mode=mode_single, hole_dia=hole_dia_single, hole_offset=hole_offset_single,
...
with tab_batch:
    ...
    if uploaded:
        try:
            ...
            st.success(f"✅ Loaded **{len(df)} rows** — required columns ✓"
                       + (f" | Optional: `{'`, `'.join(found_opt)}`" if found_opt else ""))
            ...
            svg_prev, _  = generate_svg(
                owner=str(sel_row["property_of"]), tool_number=str(sel_row["tool_number"]),
                part_desc=str(sel_row["part_description"]), mode=sel_mode,
                hole_dia=float(sel_row["hole_dia"]), hole_offset=float(sel_row["hole_offset"]),
                corner_r=corner_r, border_offset=border_offset,
                left_x=left_x, left_w=left_w, right_x=right_x,
                row1_y=row1_y, row2_y=row2_y, row3_y=row3_y,
            ...
            if st.button("🚀 Generate ZIP", type="primary", width="stretch", key="batch_gen"):
                ...
                try:
                    zb, mdf = build_batch_zip(
                        df=preview_df, default_mode=mode_default,
                        default_hole_dia=float(hole_dia_default), default_hole_offset=float(hole_offset_default),
                        corner_r=corner_r, border_offset=border_offset,
                        left_x=left_x, left_w=left_w, right_x=right_x,
                        row1_y=row1_y, row2_y=row2_y, row3_y=row3_y,
                        fs1=fs1, fs2=fs2, fs3=fs3, show_border=show_border, show_holes=show_holes,
...
# ------------------------------------------------------------
# Tab 3 — Import tool inventory (with persistent storage)
# ------------------------------------------------------------
with tab_custom:
            ...
            try:
                ...
                if is_update:
                    st.warning(
                        ...
                        f"Importing will **replace** all {sum(1 for t in store.tools if t.get('source_file') == filename)} "
                        f"rows from the old version with {n_new} new rows."
                    )
                else:
                ...
                if st.button(btn_label, type="primary", width="stretch", key="confirm_import"):
                    ...
                    st.success(
                        ...
                        f"Unchanged: {result['unchanged']} | Replaced old rows: {result.get('removed_old', 0)}"
                    )
                    st.rerun()
    ...
    # ---- Browse & export from stored inventory ----
    store.reload()
    ...
    if inv_df.empty:
        ...
        if not inv_df.empty:
            browse_labels = [
                f"P{int(r['priority'])} | {r['tool_number']} [{r.get('source_file','?')}]"
            ...
            browse_svg, _ = generate_svg(
                ...
                mode=mode_default, hole_dia=float(hole_dia_default), hole_offset=float(hole_offset_default),
                corner_r=corner_r, border_offset=border_offset,
                left_x=left_x, left_w=left_w, right_x=right_x,
                row1_y=row1_y, row2_y=row2_y, row3_y=row3_y,
                fs1=fs1, fs2=fs2, fs3=fs3, show_guides=False, show_border=show_border, show_holes=show_holes,
        ...
        # ---- Selection table for batch export ----
        ...
        edited_df     = selection_editor(full_inv, key="stored_sel_editor",
                                         display_columns=disp_cols, checkbox_label="Export")
        selected_rows = edited_df[edited_df["print"]].copy()

        ic1, ic2 = st.columns(2)
        with ic1: st.info(f"Showing: **{len(full_inv)}** | Selected: **{len(selected_rows)}**")
        with ic2:
        ...
        if selected_rows.empty:
            ...
            if st.button(f"🚀 Generate ZIP for {len(selected_rows)} rows",
                ...
                try:
                    zb, mdf = build_batch_zip(
                        df=export_df, default_mode=mode_default,
                        default_hole_dia=float(hole_dia_default), default_hole_offset=float(hole_offset_default),
                        corner_r=corner_r, border_offset=border_offset,
                        left_x=left_x, left_w=left_w, right_x=right_x,
                        row1_y=row1_y, row2_y=row2_y, row3_y=row3_y,
...
with tab_store:
    ...
    with col_stats1: st.metric("Total tools",   store.tool_count())
    with col_stats2: st.metric("Source files",  len(store.files))
    with col_stats3: st.metric("Storage path",  "laser_labels_data/")
    ...
    if files_df.empty:
        ...
        if st.button(f"🗑️ Delete `{del_choice}` and its {n_affected} tools",
                     type="secondary", key="del_file_btn"):
            store.delete_file_and_its_tools(del_choice)
            # Also delete stored upload if it exists
    ...
    if not saved_files:
        ...
        for fp in saved_files:
            ...
            with c1: st.caption(f"📄 `{fp.name}`  ({fp.stat().st_size // 1024} KB)")
            with c2:
...
with st.expander("📋 Column mapping reference"):
    st.code("""Primary: napis_na → parse:
...
Fallback columns:
    ...
    izdelek / izdelek_2      → part_description""", language="text")
